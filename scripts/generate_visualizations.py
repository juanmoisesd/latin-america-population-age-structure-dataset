import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import os
import numpy as np

def slugify(text):
    return text.lower().replace(' ', '_').replace('é', 'e').replace('ó', 'o').replace('í', 'i').replace('á', 'a').replace('ú', 'u').replace('ñ', 'n')

def main():
    df = pd.read_csv('data/population_evolution_latin_america_by_age_2000_2030_forecast.csv')
    assets_dir = 'docs/assets'
    os.makedirs(assets_dir, exist_ok=True)

    # Chart 1: Life Expectancy Trends (Major Countries)
    major_countries = ['Brasil', 'México', 'Argentina', 'Colombia', 'Chile']
    plt.figure(figsize=(10, 6))
    for country in major_countries:
        c_df = df[df['País'] == country]
        plt.plot(c_df['Año'], c_df['Esperanza_Vida'], label=country, marker='o', markersize=3)
    plt.title('Tendencia de Esperanza de Vida (2000-2030)')
    plt.xlabel('Año')
    plt.ylabel('Años')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{assets_dir}/life_expectancy_trend.png')
    plt.close()

    # Chart 2: Urbanization vs Aging (Scatter plot 2023)
    df_2023 = df[df['Año'] == 2023].copy()
    df_2023['Pct_65_mas_total'] = df_2023['Pct_65_69'] + df_2023['Pct_70_mas']
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=df_2023, x='Tasa_Urbanización', y='Pct_65_mas_total', hue='País', s=100)
    plt.title('Relación: Urbanización vs. Envejecimiento (2023)')
    plt.xlabel('Tasa de Urbanización (%)')
    plt.ylabel('% Población 65+')
    plt.savefig(f'{assets_dir}/urbanization_aging_scatter.png')
    plt.close()

    # Chart 3: Demographic Bonus (Share of 15-64 population)
    plt.figure(figsize=(10, 6))
    for country in major_countries:
        c_df = df[df['País'] == country]
        # Pct_Pob_Edad_Trabajar should be calculated by summing mid-groups or from the enriched data
        share = c_df['Pct_15_19'] + c_df['Pct_20_24'] + c_df['Pct_25_29'] + \
                c_df['Pct_30_34'] + c_df['Pct_35_39'] + c_df['Pct_40_44'] + \
                c_df['Pct_45_49'] + c_df['Pct_50_54'] + c_df['Pct_55_59'] + \
                c_df['Pct_60_64']
        plt.plot(c_df['Año'], share, label=country)
    plt.axvline(x=2023, color='grey', linestyle='--', label='Actual')
    plt.title('Bono Demográfico: % Población en Edad Laboral (15-64)')
    plt.xlabel('Año')
    plt.ylabel('% de Población')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{assets_dir}/demographic_bonus_trend.png')
    plt.close()

    # 4. Existing Pyramids & Dependency charts (Re-generate)
    for country in df['País'].unique():
        slug = slugify(country)
        c_df = df[df['País'] == country]
        # Reuse logic for dependency
        plt.figure(figsize=(8, 5))
        plt.plot(c_df['Año'], c_df['Tasa_Dependencia_Total'], label='Total')
        plt.plot(c_df['Año'], c_df['Tasa_Dependencia_Infantil'], label='Infantil', linestyle='--')
        plt.plot(c_df['Año'], c_df['Tasa_Dependencia_Vejez'], label='Vejez', linestyle='--')
        plt.title(f'Dependencia - {country}')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{assets_dir}/dependency_{slug}.png')
        plt.close()

        # Pyramid simplified for ALL
        for year in [2000, 2023]:
            row = c_df[c_df['Año'] == year]
            if not row.empty:
                r = row.iloc[0]
                age_groups = ["0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70+"]
                pct_cols = ["Pct_0_4", "Pct_5_9", "Pct_10_14", "Pct_15_19", "Pct_20_24", "Pct_25_29", "Pct_30_34", "Pct_35_39", "Pct_40_44", "Pct_45_49", "Pct_50_54", "Pct_55_59", "Pct_60_64", "Pct_65_69", "Pct_70_mas"]
                values = [r[col] for col in pct_cols]
                plt.figure(figsize=(6, 8))
                plt.barh(age_groups, values, color='teal')
                plt.title(f'{country} ({year})')
                plt.tight_layout()
                plt.savefig(f'{assets_dir}/pyramid_{slug}_{year}.png')
                plt.close()

    print("Advanced visualization suite complete.")

if __name__ == "__main__": main()
